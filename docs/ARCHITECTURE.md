...
The silhouette is not a flat black fill but two independent layers, split because one of them must never be rasterized.

`NotchGlassBackdrop` is the live glass. Where the private machinery exists it builds its own `CABackdropLayer` running the `glassBackground` filter under a signed-distance-field stack (`CASDFLayer` / `CASDFElementLayer` / `CASDFOutputEffect`), whose sublayer naming is what makes the filter refract rather than merely blur. Every symbol is resolved at runtime, so the package compiles on any SDK and `NotchCustomGlassView.isSupported` reports false wherever the machinery is absent — pre-26 systems then fall back to an `NSVisualEffectView` blur. The official wrappers are unusable here: SwiftUI's `glassEffect` samples within the window and reads flat grey inside a clear borderless panel, and `NSGlassEffectView` re-commits its filter inputs continuously, overwriting any tuning. Owning the layer and filter outright is what lets `NotchGlassStyle.glassFilterOverrides` hold. Because the window server composites this layer, it must stay outside any SwiftUI layer effect; a shader applied over it renders it blank.
