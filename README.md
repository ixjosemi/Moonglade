...
- Focus the exact Orca terminal through its stable handle. Register the Orca CLI in **Orca → Settings → General → Orca CLI** first.
...
- Orca Terminal with its CLI registered in **Orca → Settings → General → Orca CLI** for exact Orca focus.
...
| --- | --- | --- |
| Orca | exact terminal handle through `orca terminal switch --terminal <handle> --json` | Register the CLI in **Orca → Settings → General → Orca CLI**. Moonglade resolves it only from trusted standard locations or `~/.local/bin`. A missing or stale handle fails closed; it never falls back to another terminal |
...
Claude hooks, an OpenCode plugin, a Pi extension, the Codex rollout watcher, the Convoy runs watcher, and a process fallback produce versioned session documents under `~/.moonglade/state`. The integrations preserve optional Orca terminal handles and Herdr pane/socket identity when those hosts provide them; the Orca handle is validated before it can drive focus. The app observes that directory and renders active sessions. State is written atomically with user-only permissions. Convoy needs no hook at all: its run metadata under `~/.convoy/runs` is read directly, and a run is only shown while its recorded server process is verifiably alive. OpenCode phase IDs named by Convoy are retained in a private ownership index and filtered at repository load time, so internal phases stay hidden even after a plugin rewrite or app restart.
...
Moonglade has no networking or telemetry. It stores local session metadata—including project paths, process IDs, timestamps, terminal identifiers, and optional Orca handles—but not prompts or model responses. Read [PRIVACY.md](PRIVACY.md) before installing integrations and [SECURITY.md](SECURITY.md) before reporting a vulnerability.
